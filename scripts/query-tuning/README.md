...
- **exporter mysql 메트릭 끊김** → 컨테이너가 `local_local-monitoring` 네트워크에 없거나 127.0.0.1 바인딩 후 `host.docker.internal`로 붙는 옛 설정. `provision-level.sh`로 생성 + exporter는 `<컨테이너명>:3306` 접근(run.sh 의 run_level 이 처리).
