#!/usr/bin/env bash
# =============================================================
# restore-x1.sh — 마스킹 덤프를 x1 컨테이너에 복원 + 정합성 검증 (query-tuning x1 세팅).
#   무인자면 최신 마스킹본을 자동으로 내려받아 복원한다(다운로드는 download-masked.sh --latest 에 위임).
...
if [[ -z "$DUMP" ]]; then
  echo "[restore-x1] 인자 없음 — 최신 마스킹본 자동 다운로드(download-masked.sh --latest)"
  DUMP="$(bash "$DIR/download-masked.sh" --latest)" \
    || { echo "[restore-x1] 최신 마스킹본 다운로드 실패 — 버킷 masked/ 가 비었으면 masked-export 먼저" >&2; exit 1; }
fi
