...
/** 빈칸채우기(BLANK) 퀴즈 유형. 생성 엔진은 {@link ChunkedQuizGenerator} 하나를 공유하고, 이 클래스는 이 유형의 차이만 정의한다. */
@Component
@RequiredArgsConstructor
public class BlankQuizOrchestrator implements QuizTypeOrchestrator, QuizTypeSpec<GeminiQuestion> {
  ...
  public String systemGuideLine(String language) {
    return GuideLines.withLanguage(BlankGuideLine.content, language);
  }

  @Override
  public String requestPrompt(int quizCount, String customInstruction) {
    return BlankRequestPrompt.generate(quizCount, customInstruction);
  }
