...
/**
 * customInstruction 유무·acceptedAnswers 노출 여부에 따라 적절한 JSON 스키마를 제공한다.
 *
 * <p>지시사항이 없으면 appliedInstruction 필드를, REAL_BLANK가 아니면 acceptedAnswers 필드를 스키마에서 제외하여 Gemini가 불필요한
 * 값을 생성하지 않도록 한다. GeminiResponse 스키마 하나에서 파생하므로 필드 정의 중복이 없다.
 */
...
public class GeminiResponseSchema {
  ...
  /** customInstruction 유무·acceptedAnswers 노출 여부에 따라 적절한 스키마를 반환한다. */
  public static String forInstruction(String customInstruction, boolean includeAcceptedAnswers) {
    ...
    if (hasInstruction) {
      return includeAcceptedAnswers ? WITH_INSTRUCTION : WITH_INSTRUCTION_NO_ACCEPTED;
    }
    return includeAcceptedAnswers ? WITHOUT_INSTRUCTION : WITHOUT_INSTRUCTION_NO_ACCEPTED;
  }

  /** JSON 스키마에서 지정 프로퍼티와 required 항목을 제거한다. */
