 ...
 *   <li><b>일부 Update(partial column)</b>: 변경 행의 UPDATE SET 절에 review만 있고, 미변경 컬럼(v1_explanation 등)은
 *       재기록되지 않는가.
 * </ul>
 */
@TestPropertySource(
...
class EnhancementWriteScenarioTest extends JpaIntegrationTestBase {
  ...
  void partialUpdateAndDirtyTrackingInOneScenario() {
    ...
    assertThat(updates)
        .allSatisfy(
            sql -> {
              assertThat(sql).as("변경 컬럼 포함").contains("review");
              assertThat(sql)
                  .as("미변경 컬럼(v1_explanation)은 UPDATE에 포함되지 않아야 함")
                  .doesNotContain("v1_explanation");
            });
  }
