...
class MockAIServerAdapterTest {
  ...
  private List<AIProblem> generate(QuizType quizType, int quizCount) {
  ...
  void real_blank_strategy_emits_real_blank_mocks() {
    List<AIProblem> problems = generate(QuizType.REAL_BLANK, 5);
  ...
  void choice_strategy_keeps_four_selection_mocks() {
    List<AIProblem> problems = generate(QuizType.MULTIPLE, 3);
