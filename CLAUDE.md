...
│       ├── db/migration/         # Flyway 마이그레이션 SQL (V1~V21)
│       └── config/               # 분리된 설정 파일들
...
│   ├── board/    (api + impl)    # 게시판
│   ├── quiz-ai/  (api + impl)    # AI 퀴즈 생성 (Gemini 호출, 청크 분할 스트리밍 — 유형 공통 엔진 ChunkedQuizGenerator를 유형별 QuizTypeOrchestrator/QuizTypeSpec 구현(blank/essay/multiple/ox/realblank 하위 패키지)이 공유하고 QuizOrchestratorServiceImpl이 QuizType enum으로 디스패치, 저장·SSE 전송은 quiz-make의 QuizConsumer 구현이 담당)·컨텍스트 캐시(ChunkPlanner/GeminiContextCacheManager — 생성 성패를 gemini.cache.create 카운터로 기록, 실패=프리픽스 매 호출 재전송 강등), 메트릭, 품질 검증 QualityVerifier(ChunkedQuizGenerator가 직접 verify·재생성 루프를 돈다, 별도 게이트 클래스 없음) — 검증관 프롬프트는 생성 GuideLine이 아니라 유형별 관찰 가능 실격 사유(QualityVerifierImpl.DISQUALIFIERS)로 독립 판정하고, 검증 항목은 유형에 맞게 필터링(ESSAY 전용↔객관형 전용 분리))
│   ├── quiz-make/(api + impl)    # 퀴즈 생성 흐름 (파일업로드, SSE, 생성결과)
│   ├── quiz-set/ (api + impl)    # 퀴즈 세트 CRUD, 품질 리뷰(QualityReviewService, problemSetIds 배치 재검토)·해설 재검토(ExplanationReviewService/ExplanationFormatValidator)·품질 로그(QualityLogService/ProblemQualityLog, v1 생성본·v2 재생성본 함께 보관)·스테일 생성 복구(StaleGenerationRecovery 인터페이스 — createdAt 이 15분 지난 FAILED·GENERATING 세트를 부모 벌크 JPQL delete 로 삭제(임계는 생성 세션 시한 10분보다 길어야 한다 — 같으면 진행 중인 생성 세트가 저장 직전에 지워진다), 자식 problem 은 FK ON DELETE CASCADE[V19]가 DB 에서 자동 삭제; mock 구현은 동일 SELECT+벌크 delete 를 태운 뒤 롤백해 순증 0)·오답 세트 생성(WrongAnswerSetCreationService — AI 를 거치지 않고 원본 Problem 을 그대로 복제, ProblemSet.origin=DOCUMENT|WRONG_ANSWER[V21]·source_folder_id 로 출처를 남기고 Problem.origin_problem_set_id·origin_number 에 최초 조상 혈통을 물려줘 세대가 반복돼도 같은 값이 되게 한다(중복 제거 키); 원본 자료를 가리키는 페이지 번호는 물려주지 않는다 — 자료 없는 세트에서 해설의 참조 안내가 사실과 달라진다. 호출자 트랜잭션에 참여(새 트랜잭션을 열지 않는다))
│   ├── quiz-history/(api + impl) # 풀이 히스토리 + 기록 폴더 분류(QuizFolder 엔티티, /folders CRUD[POST·GET·PATCH·DELETE], PATCH /history/{id}/folder 배정·해제, GET /history?scope=ALL|UNCLASSIFIED|FOLDER&folderId= 필터링; QuizFolderCommand/QueryService, QuizHistory.folder_id) + 오답 모아풀기(POST /problem-set/wrong-answers {folderId}, 선택 헤더 Idempotency-Key — WrongAnswerSetService/Impl 이 내 기록∩폴더∩COMPLETED 한 쿼리로 수집 범위를 닫는다. 정오답은 저장돼 있지 않아 AnswerJudge(quizhistory.grading, 기록 상세와 공용 판정 SSOT — REAL_BLANK 는 RealBlankGrader, 그 외는 정답 인덱스 비교; ESSAY 는 규칙으로 갈리지 않아 제외하고 제외 문항 수만 응답)로 되짚고, WrongAnswerCollector 가 최근 푼 순 정렬 → 혈통 중복 제거 → 유형별 상한 100(MAX_QUESTIONS_PER_SET) 순서로 유형별 구성을 만든다(정렬이 앞서야 상한에서 최신 오답이 남는다). WrongAnswerSetFactory 는 유형마다 REQUIRES_NEW 로 문제집 + 미완료 풀이 기록을 출처 폴더에 함께 만들어(폴더는 문제집이 아니라 기록에 붙고 목록도 기록을 훑으므로 기록이 없으면 목록·폴더에 안 뜬다) 한 유형이 실패해도 나머지가 남게 하고, sessionId="wa-{키}-{유형}" 이 UNIQUE 라 연타·재시도는 기존 세트를 돌려준다. 빈 결과·상한 초과·부분 실패는 모두 200 + emptyReason/failedTypes; QuizHistory.completed_at[V21]이 목록의 takenAt=완료 시각이라 미완료 기록은 null)
