...
| 포맷터           | Spotless + Google Java Format                                         | 7.0.4 / 1.35.0 |
...
│   └── admin/                    # 관리자 전용 API
├── infra/
│   ├── monitoring/               # Grafana Alloy 설정 (alloy/), Grafana 대시보드/룰 (grafana/), Prometheus rules (prometheus/)
...
- OCI Object Storage: `~/.oci/config` 파일 기반 인증, `oci.object-storage.namespace` /
  `oci.object-storage.image-bucket-name` / `oci.object-storage.pdf-bucket-name` (
  `application-secrets.yml` ENC 관리)
- Google Cloud: Vertex AI + GCS (ADC 인증)
    - `spring.ai.google.genai.project-id`: GCP 프로젝트 ID (`application-secrets.yml` ENC 관리)
    - `spring.ai.google.genai.location`: GCP 엔드포인트 (`application-secrets.yml` ENC 관리)
    - `q-asker.ai.gcs.bucket-name`: GCS 버킷 이름 (`application-secrets.yml` ENC 관리)
    - 로컬: `gcloud auth application-default login`, 프로덕션: 서비스 계정
- 로컬 MySQL: 프로덕션 설정과 정렬 (`innodb-buffer-pool-size=4G`, `innodb-buffer-pool-instances=4`,
  `innodb-flush-log-at-trx-commit=1`, `max-connections=200`, `innodb-io-capacity=625`,
  `innodb-io-capacity-max=10000`,
  `innodb-read-io-threads=1`, `innodb-write-io-threads=4`, `infra/mysql/docker-compose.yml`)
...
- **포맷터**: Spotless + Google Java Format 1.35.0
